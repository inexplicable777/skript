#!/bin/sh

# ============================================
# Только настройка awg10 (AmneziaWG WARP)
# ============================================

INTERFACE_NAME="awg10"
CONFIG_NAME="amneziawg_awg10"
PROTO="amneziawg"
ZONE_NAME="awg"

# ====================== ВВОД ПАРАМЕТРОВ ======================

echo "=== Настройка AmneziaWG (awg10) ==="

read -r -p "Ввести параметры вручную? (y/n): " is_manual

if [ "$is_manual" = "y" ] || [ "$is_manual" = "Y" ]; then
    read -r -p "PrivateKey: " PrivateKey
    read -r -p "S1: " S1
    read -r -p "S2: " S2
    read -r -p "Jc: " Jc
    read -r -p "Jmin: " Jmin
    read -r -p "Jmax: " Jmax
    read -r -p "H1: " H1
    read -r -p "H2: " H2
    read -r -p "H3: " H3
    read -r -p "H4: " H4
    read -r -p "Address (например 172.16.0.2/32): " Address
    read -r -p "PublicKey: " PublicKey
    read -r -p "Endpoint host (IP или домен): " EndpointIP
    read -r -p "Endpoint port [500]: " EndpointPort
    EndpointPort=${EndpointPort:-500}
    MTU=1280
else
    echo "Автоматический запрос конфига WARP..."
    # Здесь можно оставить вызовы requestConfWARP* по необходимости
    # Для примера — заглушка. Замените на нужный генератор.
    echo "Ошибка: автоматический режим пока отключён в упрощённой версии."
    echo "Используйте ручной ввод (y)."
    exit 1
fi

# ====================== UCI НАСТРОЙКА ======================

uci set network.${INTERFACE_NAME}=interface
uci set network.${INTERFACE_NAME}.proto=$PROTO

if ! uci show network | grep -q ${CONFIG_NAME}; then
    uci add network ${CONFIG_NAME}
fi

uci set network.${INTERFACE_NAME}.private_key="$PrivateKey"
uci del network.${INTERFACE_NAME}.addresses 2>/dev/null
uci add_list network.${INTERFACE_NAME}.addresses="$Address"
uci set network.${INTERFACE_NAME}.mtu=$MTU
uci set network.${INTERFACE_NAME}.awg_jc=$Jc
uci set network.${INTERFACE_NAME}.awg_jmin=$Jmin
uci set network.${INTERFACE_NAME}.awg_jmax=$Jmax
uci set network.${INTERFACE_NAME}.awg_s1=$S1
uci set network.${INTERFACE_NAME}.awg_s2=$S2
uci set network.${INTERFACE_NAME}.awg_h1=$H1
uci set network.${INTERFACE_NAME}.awg_h2=$H2
uci set network.${INTERFACE_NAME}.awg_h3=$H3
uci set network.${INTERFACE_NAME}.awg_h4=$H4

# I1 (длинный параметр AmneziaWG) — можно оставить дефолт или задать свой
uci set network.${INTERFACE_NAME}.awg_i1="<b >"

uci set network.${INTERFACE_NAME}.nohostroute='1'

uci set network.@${CONFIG_NAME}[-1].description="${INTERFACE_NAME}_peer"
uci set network.@${CONFIG_NAME}[-1].public_key="$PublicKey"
uci set network.@${CONFIG_NAME}[-1].endpoint_host="$EndpointIP"
uci set network.@${CONFIG_NAME}[-1].endpoint_port="$EndpointPort"
uci set network.@${CONFIG_NAME}[-1].persistent_keepalive='25'
uci set network.@${CONFIG_NAME}[-1].allowed_ips='0.0.0.0/0'
uci set network.@${CONFIG_NAME}[-1].route_allowed_ips='0'

uci commit network

# ====================== FIREWALL ======================

if ! uci show firewall | grep -q "@zone.*name='${ZONE_NAME}'"; then
    uci add firewall zone
    uci set firewall.@zone[-1].name=$ZONE_NAME
    uci set firewall.@zone[-1].network=$INTERFACE_NAME
    uci set firewall.@zone[-1].forward='REJECT'
    uci set firewall.@zone[-1].output='ACCEPT'
    uci set firewall.@zone[-1].input='REJECT'
    uci set firewall.@zone[-1].masq='1'
    uci set firewall.@zone[-1].mtu_fix='1'
    uci set firewall.@zone[-1].family='ipv4'
    uci commit firewall
fi

if ! uci show firewall | grep -q "@forwarding.*name='${ZONE_NAME}'"; then
    uci add firewall forwarding
    uci set firewall.@forwarding[-1].name="${ZONE_NAME}"
    uci set firewall.@forwarding[-1].dest=$ZONE_NAME
    uci set firewall.@forwarding[-1].src='lan'
    uci set firewall.@forwarding[-1].family='ipv4'
    uci commit firewall
fi

# Добавляем интерфейс в зону (на всякий случай)
for zone in $(uci show firewall | grep "zone$" | cut -d'=' -f1); do
    if [ "$(uci get $zone.name)" = "$ZONE_NAME" ]; then
        if ! uci get $zone.network | grep -q "$INTERFACE_NAME"; then
            uci add_list $zone.network="$INTERFACE_NAME"
            uci commit firewall
        fi
    fi
done

echo "Конфиг awg10 успешно применён."
echo "Перезапускаем сеть и firewall..."

ifdown $INTERFACE_NAME 2>/dev/null
ifup $INTERFACE_NAME

service firewall restart
service network restart

echo "Готово! Проверьте статус: ifstatus awg10"
